...
class LinkedListEntry {
	...
	public function __construct($element, LinkedListEntry $next = null, LinkedListEntry $previous = null) {
...
/** @gwtname java.util.LinkedHashSet */
...
class MapEntryIterator implements Iterator {
	private $keys;
	private $values;
	...
	public function current() {
		$entry = new SimpleEntry(current($this->keys), current($this->values));
		return $entry;
	}
...
abstract class AbstractMap implements Map {
	...
	public function __toString() {
		$result = '[';
		foreach ($this->entryIterator() as $entry) {
			$result .= $entry . ', ';
		}
		$len = mb_strlen($result);
		if ($len != 1) {
			$result = mb_substr($result, 0, $len - 2);
		}
		return $result . ']';
	}
}
...
class HashMap extends AbstractMap {
	...
	public function entryIterator() {
		return new MapEntryIterator($this->keys, $this->values);
	}
}
...
/** @gwtname java.util.LinkedHashMap */
